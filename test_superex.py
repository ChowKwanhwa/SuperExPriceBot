...
class SuperExAPI:
    def __init__(self):
        ...
        # 添加缓存
        self._cache = {}
        self._cache_timeout = 10  # 缓存超时时间（秒）
    ...
    def _update_market_data(self):
        """更新所有市场数据到缓存"""
        now = datetime.now()
        # 如果距离上次更新不到10秒，直接返回缓存数据
        if self._last_full_update and (now - self._last_full_update).total_seconds() < self._cache_timeout:
            return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("code") == 200 and "data" in data:
                    # 更新缓存
                    self._market_data = {item["trading_pairs"]: item for item in data["data"]}
        ...
        try:
            ...
            if symbol in self._market_data:
                ...
                return {
                    ...
                    "24h_high": float(data["highest_price_24h"]),
                    "24h_low": float(data["lowest_price_24h"]),
                    "24h_volume": float(data["base_volume"]),
                    "24h_change": float(data["price_change_percent_24h"])
                }
            else:
